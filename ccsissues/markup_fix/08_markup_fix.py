"""
Markup fixer + audit for ccs.txt (CCS).
Counterpart of PWG#175, PWK#113, and siblings.

CCS-specific notes: No paired tags. Completely clean.

Inputs: ../../../csl-orig/v02/ccs/ccs.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "ccs" / "ccs.txt", HERE / "ccs.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "ccs_fixed.txt"
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for ccs.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"CCS markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against ccs.txt.\n")
